...
test("pricing retains the approved plans and uses the secure checkout client", () => {
  ...
  const css = read("app/globals.css");
  assert.match(pricing, /<span className="section-kicker">SIMPLE PRICING<\/span>[\s\S]*<h1 className="page-section-title">Choose Your Plan<\/h1>/);
  assert.doesNotMatch(pricing, /PLAN UPGRADES|Pick What Suits You|Start with five questions a day/);
