...
const apiOrigin = requiredApiOrigin(process.env.NEXT_PUBLIC_API_URL);
...
export const config = {
  matcher: [
    // Only Next's guaranteed internal asset handlers bypass the proxy. A
    // missing public asset (for example /favicon.ico) is rendered through the
    // app not-found boundary and therefore still needs the request nonce used
    // by the root layout. Excluding arbitrary file extensions made those 404s
    // crash server rendering with "The request CSP nonce is missing."
...
function buildContentSecurityPolicy(nonce: string): string {
  return [
    ...
    "font-src 'self' data:",
    `connect-src 'self' ${apiOrigin} ${clerkFrontendApiOrigin} https://*.protect.clerk.com https://clerk-telemetry.com https://*.clerk-telemetry.com`,
