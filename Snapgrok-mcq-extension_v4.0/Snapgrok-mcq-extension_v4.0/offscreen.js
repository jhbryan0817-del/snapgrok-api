...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (!operationId || !serverUrl || !requestBody || typeof requestBody !== "object") {
    sendResponse({ accepted: false, error: "The background analysis request was invalid." });
    return false;
  }

  sendResponse({ accepted: true });
  void runAnalysis({ operationId, serverUrl, timeoutMs, requestBody });
  return false;
});

async function runAnalysis({ operationId, serverUrl, timeoutMs, requestBody }) {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), timeoutMs);
  activeRequest = { operationId, controller };
