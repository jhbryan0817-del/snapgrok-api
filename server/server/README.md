...
v5.8.1 is a localized reconciliation hardening release. It safely ignores an
older adverse-payment replay after a newer refund or dispute webhook has
already changed the membership, never sends an empty checkout-intent value to
a PostgreSQL UUID comparison, and retains only the five-character SQLSTATE in
maintenance diagnostics. It does not change checkout, quota, pricing,
entitlement, website, or extension behavior.
