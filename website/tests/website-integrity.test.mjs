...
test("logo and viewport motion refinements remain stable", () => {
  ...
  assert.match(logo, /className="brand-logo-stacked" src="\/zenaian-logo-full\.png"/);
  assert.doesNotMatch(logo, /brand-logo-icon|brand-logo-wordmark/);
  assert.match(css, /\.site-header \{[^}]*height: 78px;[^}]*min-height: 78px;/);
  assert.match(css, /\.brand \.brand-logo \{[^}]*width: 61px;[^}]*height: 50px;/);
  assert.match(css, /\.footer-brand \.brand-logo-compact \{[^}]*width: 48px;[^}]*height: 39px;/);
