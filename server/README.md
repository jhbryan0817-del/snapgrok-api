...
- `/api/analyze` and `/api/balance` require a valid Clerk session token and an
...
- After local JWT verification, the server asks Clerk for the current session
  status on every protected request. Ended or revoked sessions fail immediately
  instead of retaining access until the short-lived JWT expires.
...
- If the extension cancels or disconnects, the server propagates that abort to
  the in-flight xAI request and releases the user's concurrency slot.
...
`\n` characters if Render stores it on one line), JWT signature verification is
local. The active-session check still uses Clerk's Backend API and therefore
requires `CLERK_SECRET_KEY` and network access. The API fails closed with HTTP
503 if Clerk cannot confirm session state.
...
Unauthenticated or inactive-session requests receive HTTP 401. Invalid origins receive HTTP 403.
