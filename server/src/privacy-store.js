...
export function createPostgresPrivacyStore({
  ...
  return {
    async initialize() {
      ...
      try {
        const readiness = await inspectPrivacyRuntimeReadiness(client);
        if (!readiness.ready) {
          const error = privacyStoreError(
            "Privacy database runtime readiness validation failed.",
            readiness.code || "PRIVACY_DATABASE_READINESS_FAILED",
          );
          if (readiness.databaseCode) {
            error.databaseCode = readiness.databaseCode;
          }
        ...
        await client.query(
          `UPDATE privacy_request_audit
           SET state = 'complete', completed_at = $2, exception_code = NULL,
               purge_after = $2::timestamptz + interval '1 year',
    ...
    async recordDeletionReplayBlock({ requestId, userId, completedAt }) {
      requireUserId(userId);
      const at = asDate(completedAt);
      if (!/^[0-9a-f-]{36}$/i.test(String(requestId || "")) || !at) {
        throw privacyStoreError(
          "Deletion replay input is invalid.",
          "PRIVACY_DELETION_REPLAY_INVALID",
          400,
        );
      }
      const subjectHmacs = userHmacCandidates(userId);
      const currentSubjectHmac = userHmac(userId);
      const client = await database.connect();
      try {
        await client.query("BEGIN");
        await client.query(
          `INSERT INTO privacy_request_audit (
             request_id, subject_hmac, request_type, state, received_at,
             completed_at, purge_after, updated_at
           ) VALUES (
             $1, $2, 'delete', 'complete', $3, $3,
             $3::timestamptz + interval '1 year', $3
           )
           ON CONFLICT DO NOTHING`,
          [requestId, currentSubjectHmac, at],
        );
        ...
        if (blocked.rowCount < 1) {
          throw privacyStoreError(
            "Deletion replay could not establish the durable access block.",
            "PRIVACY_DELETION_REPLAY_BLOCK_FAILED",
          );
        }
        await client.query("COMMIT");
        return true;
    ...
    async finishDeletionReplay(userId) {
      requireUserId(userId);
      try {
        const result = await database.query(
          `DELETE FROM privacy_deletion_queue
           WHERE clerk_user_id = $1`,
          [userId],
        );
    ...
    async getDeletionBacklog(at = new Date()) {
      try {
        const result = await database.query(
    ...
    async getZdrSafetyState() {
      try {
        ...
        if (!result.rows[0]) {
          throw privacyStoreError(
            "The xAI ZDR safety latch is unavailable.",
            "XAI_ZDR_LATCH_UNAVAILABLE",
          );
        }
        return safetyLatchState(result.rows[0]);
      } catch (error) {
        throw normalizePrivacyStoreError(error);
      }
    },

    async recordZdrFailure(threshold, at = new Date()) {
      const boundedThreshold = Math.max(
        2,
        Math.min(20, Number.isSafeInteger(threshold) ? threshold : 3),
      );
      try {
        const result = await database.query(
    ...
    async recordZdrSuccess(at = new Date()) {
      try {
        const result = await database.query(
          `UPDATE runtime_safety_latches
           SET consecutive_failures = 0,
               last_success_at = $1,
               updated_at = $1
           WHERE latch_name = 'xai_zdr' AND state = 'enabled'
           RETURNING state, consecutive_failures, disabled_at,
                     last_failure_at, last_success_at`,
          [at],
        );
        if (!result.rows[0]) return this.getZdrSafetyState();
        return safetyLatchState(result.rows[0]);
      } catch (error) {
        throw normalizePrivacyStoreError(error);
      }
        ...
        const result = await database.query(
          `UPDATE legal_retention.transaction_records
           SET status = 'cancel_at_period_end',
               ...
               retention_expires_at = GREATEST(
                 retention_expires_at,
                 $4::timestamptz + interval '5 years'
               ),
               updated_at = $4
           WHERE provider = 'whop'
             AND provider_mode = $1
      ...
      try {
        counts.analysisUsage = await deleteBatch(database,
          `SELECT usage.operation_id FROM billing_analysis_usage AS usage
           WHERE usage.state IN ('consumed', 'released')
             AND usage.settled_at < $1::timestamptz - interval '30 days'
          ...
          `target.state IN ('consumed', 'released')
           AND target.settled_at < $1::timestamptz - interval '30 days'`);
        counts.usagePeriods = await deleteBatch(database,
          `SELECT period.id FROM billing_usage_periods AS period
           WHERE period.ends_at < $1::timestamptz - interval '90 days'
          ...
          `target.ends_at < $1::timestamptz - interval '90 days'
           AND NOT EXISTS (
        ...
        counts.checkouts = await deleteBatch(database,
          `SELECT checkout.id FROM billing_checkout_sessions AS checkout
           WHERE (
             (checkout.status IN ('failed', 'expired')
              AND checkout.updated_at < $1::timestamptz - interval '7 days') OR
          ...
          `(
             (target.status IN ('failed', 'expired')
              AND target.updated_at < $1::timestamptz - interval '7 days') OR
        ...
        const providerEvents = await database.query(
          `WITH doomed AS (
             ...
             WHERE candidate.received_at < $1::timestamptz - interval '30 days'
           ...
           WHERE event.provider = doomed.provider
             ...
             AND event.received_at < $1::timestamptz - interval '30 days'`,
          [at, limit],
        );
        ...
        const paymentHistory = await database.query(
          `WITH doomed AS (
             ...
             WHERE candidate.archived_at IS NOT NULL
               AND candidate.provider_updated_at <
                   $1::timestamptz - interval '12 months'
               AND EXISTS (
                 SELECT 1
                 FROM legal_retention.transaction_records AS archive
           ...
           WHERE payment.provider_mode = doomed.provider_mode
             ...
             AND payment.provider_updated_at <
                 $1::timestamptz - interval '12 months'
             AND EXISTS (
               SELECT 1
               FROM legal_retention.transaction_records AS archive
        ...
        const memberships = await database.query(
          `WITH doomed AS (
             ...
             WHERE membership.access_state IN ('inactive', 'revoked')
               AND membership.state_changed_at <
...
function safetyLatchState(row) {
  const state = String(row?.state || "");
  if (!new Set(["enabled", "disabled"]).has(state)) {
    throw privacyStoreError(
      "The xAI ZDR safety latch has an invalid state.",
      "XAI_ZDR_LATCH_INVALID",
    );
  }
  return {
    state,
    consecutiveFailures: Number(row.consecutive_failures || 0),
    disabledAt: asDate(row.disabled_at)?.toISOString() || null,
    lastFailureAt: asDate(row.last_failure_at)?.toISOString() || null,
    lastSuccessAt: asDate(row.last_success_at)?.toISOString() || null,
  };
}
