...
export async function analyzeScreenshot({
  ...
  if (!apiKey || apiKey === "paste_your_xai_api_key_here") {
    throw new Error("XAI_API_KEY is missing. Add it to the server environment and restart the service.");
  }
  ...
  // store:false instructs xAI not to retain this response through the API's
  // response-storage feature. The screenshot is used only for this request.
  const requestBody = {
    model,
    store: false,
