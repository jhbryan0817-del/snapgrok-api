# SneakSolve v5.1.5 acceptance tests
...
33. Inspect an authenticated `/api/analyze` request and confirm it contains one
    UUID `operationId`, then confirm an xAI success consumes exactly one unit.
34. Force an xAI technical failure and confirm the reserved unit is released.
35. Use all five Free questions and confirm the sixth capture does not call xAI
    and the toolbar tooltip explains the daily limit and reset time.
36. Confirm multiple-answer results still display for six seconds, single
