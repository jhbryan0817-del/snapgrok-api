...
test("global header has the requested destinations", () => {
  ...
  for (const label of ["Pricing", "Privacy Policy"]) {
    assert.match(header, new RegExp(`>\\s*${label}\\s*<`));
  }
  assert.doesNotMatch(header, />Home<|>Why SneakSolve<|>Account|Affiliate Marketing/);
...
test("global footer contains navigation and accurate xAI trademark attribution", () => {
  ...
  for (const label of ["Pricing", "Privacy Policy", "Account"]) {
...
test("answer-state icons retain the requested artwork and balanced sizing", () => {
  ...
  const inconclusive = readBytes(
    "public/sneaksolve-icons/result-inconclusive.png",
  );
  assert.equal(
    createHash("sha256").update(inconclusive).digest("hex"),
    "e3f4f3a074e6e68805d750ed331d846e2370fbec557a4e2780c06f8f7ba4d836",
    "The website must reuse the reviewed extension inconclusive icon.",
  );
  assert.equal(inconclusive.readUInt32BE(16), 128);
  assert.equal(inconclusive.readUInt32BE(20), 128);
  assert.match(
    page,
    /result-inconclusive\.png"[\s\S]*iconClass: "state-icon-compact"/,
  );
  assert.match(
    page,
    /result-error\.png"[\s\S]*iconClass: "state-icon-compact"/,
  );
  assert.match(
    css,
    /\.state-card img\.state-icon-compact[\s\S]*width: 48px[\s\S]*height: 48px[\s\S]*margin: 4px/,
  );
});
...
test("pricing retains the approved plans and uses the secure checkout client", () => {
  ...
  assert.match(action, /subscription-change handling is added/);
  assert.doesNotMatch(action, /LEMONSQUEEZY_(?:API_KEY|WEBHOOK_SECRET)/);
});

test("account plan panel reads server-authoritative status and portal URLs", () => {
  ...
  assert.match(panel, /createCustomerPortal/);
...
test("account readiness combines extension installation and server quota state", () => {
  ...
  assert.match(readiness, /pjfanaeopegobidkbpnlmeegmkmnabmk/);
...
test("privacy page covers the current product and test-billing data flow", () => {
  const privacy = read("app/privacy/page.tsx");
  for (const expected of ["Clerk", "Render", "xAI", "Lemon Squeezy", "PostgreSQL", "captured screenshots", "Working draft"]) {
