...
export function createAuthenticator({
  ...
  return async function authenticate(incomingRequest) {
    ...
    return {
      ...
      factorVerificationAge: normalizeFactorVerificationAge(
...
function normalizeFactorVerificationAge(value) {
  if (!Array.isArray(value) || value.length !== 2) return null;
  const normalized = value.map((age) => {
    if (age === -1) return -1;
    const numeric = Number(age);
    return Number.isFinite(numeric) && numeric >= 0 ? numeric : null;
  });
  return normalized.every((age) => age !== null) ? normalized : null;
}
