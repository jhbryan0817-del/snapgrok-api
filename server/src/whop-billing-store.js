  ...
  return {
    ...
    async createCheckoutIntent({
      ...
      try {
        ...
        if (memberships.rows.some((row) => row.plan_code === planCode)) {
          throw billingConflict(
            `This account already has an active ${planCode} subscription.`,
            "SUBSCRIPTION_ALREADY_ACTIVE",
          );
        }
        if (
          planCode === "plus" &&
          memberships.rows.some((row) => row.plan_code === "ultra")
        ) {
          throw billingConflict(
            "Plus cannot be purchased while Ultra is active.",
          ...
          const openMembership = await client.query(
            `SELECT provider_membership_id FROM billing_memberships
             WHERE provider_mode = $1 AND clerk_user_id = $2
               AND access_state IN ('active', 'cancel_at_period_end')
               AND (
                 plan_code = $3 OR ($3 = 'plus' AND plan_code = 'ultra')
               )
             LIMIT 1`,
            [providerMode, userId, membership.planCode],
          );
        ...
        if (
          ...
          const conflictingRestore = await client.query(
            `SELECT provider_membership_id
             ...
             WHERE provider_mode = $1 AND clerk_user_id = $2
               AND plan_code = $3
               AND provider_membership_id <> $4
               AND access_state IN ('active', 'cancel_at_period_end')
             LIMIT 1`,
            [providerMode, userId, membership.planCode, membership.id],
          );
