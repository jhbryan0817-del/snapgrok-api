...
test("redesigned landing page includes the requested product workflow", () => {
  ...
  assert.match(page, /subject-chips[\s\S]*>Psychology<[\s\S]*>Anatomy</);
  assert.match(page, /review-track-manual[\s\S]*Screenshot[\s\S]*Switch Screen[\s\S]*Paste and Ask[\s\S]*Confirm[\s\S]*Return/);
  ...
  assert.match(page, /FRONTIER INTELLIGENCE/);
  assert.match(page, /frontier-panel[\s\S]*frontier-model-mark[\s\S]*Precise reasoning/);
  assert.doesNotMatch(page, /study-intelligence-panel|study-speed-card|study-model-card|SWE Marathon|benchmark-visual/);
...
test("pricing retains the approved plans and uses the secure checkout client", () => {
  ...
  const css = read("app/globals.css");
  assert.match(pricing, /<h1 className="page-section-title">Plan Upgrades<\/h1>/);
...
test("account readiness combines extension installation and server quota state", () => {
  ...
  assert.match(account, /account-welcome account-status-only/);
  assert.match(account, /<h1 className="page-section-title">Account Settings<\/h1>[\s\S]*<AccountReadiness \/>/);
