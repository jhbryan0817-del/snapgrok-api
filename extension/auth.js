(() => {
  ...
  async function getAuthSnapshot({ verify = true } = {}) {
    ...
    try {
      const response = await fetchWithAuth("/api/extension/session", {
        method: "GET",
      });
  ...
  async function refreshSession(existingSession = null) {
    ...
    refreshPromise = (async () => {
      ...
      if (!refreshed) {
        await clearSession();
        throw new Error("SneakSolve received an invalid refreshed session.");
      }
  ...
  async function fetchWithAuth(path, init = {}) {
    ...
    if (!token) {
      throw authError(
        ...
        "Sign in to SneakSolve from the extension popup before capturing a question.",
      );
    }
    ...
    throw authError(
      "DEVICE_SESSION_REQUIRED",
      "Your SneakSolve session is no longer active.",
    );
  }
  ...
  async function acceptPairing({ pairingCode, nonce }) {
    ...
    if (
      ...
      throw authError(
        ...
        "The extension connection request expired. Reopen SneakSolve and try again.",
      );
    }
    ...
    if (!session) {
      throw new Error("SneakSolve received an invalid extension session.");
    }
  ...
  function normalizeProfile(value) {
    ...
    return {
      accountId,
      email,
      displayName: displayName || email || "SneakSolve user",
    };
  }
