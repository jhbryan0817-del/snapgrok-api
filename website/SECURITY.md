...
of the SneakSolve API billing endpoints. It contains only public
`NEXT_PUBLIC_*` configuration. Clerk owns the browser session and its
...
4. The website navigates only to a validated HTTPS Lemon Squeezy URL returned
   by that endpoint. It does not accept arbitrary `return_to`, checkout, or
   portal URLs.
