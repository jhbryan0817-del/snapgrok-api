importScripts("auth-config.js", "clerk-auth.js", "auth-sync-fix.js", "settings.js", "protocol.js");
...
function trace(eventName, details = {}) {
  ...
  // output, and answers.
  console.debug(`[SnapGrok V3.9] ${eventName}`, details);
}
...
async function closeOffscreenDocumentQuietly() {
  try {
    ...
    console.debug(`[SnapGrok V3.9] Offscreen document cleanup skipped: ${error?.message || "Unknown error"}`);
  }
}
...
async function displayOutcome(operationId, rawOutcome, error = null) {
  ...
  if (error) {
    console.error(
      `[SnapGrok V3.9] ${error?.name || "Error"}: ${error?.message || "Unknown error"}`,
    );
  }
