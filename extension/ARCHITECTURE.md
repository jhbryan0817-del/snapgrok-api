# SneakSolve v5.0.1 architecture

1. The auth bridge checks the production Clerk Sync Host cookie and creates a
   fresh background Clerk client.
2. The service worker requires a current session token before any screenshot is
   captured.
