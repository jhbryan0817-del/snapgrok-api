...
test("redesigned landing page includes the requested product workflow", () => {
  ...
  assert.match(page, /zenaian-how-it-works-v2\.png/);
  ...
  assert.match(page, /heroAnswerBadges/);
  assert.match(page, /result-b128\.png/);
  assert.match(page, /result-e128\.png/);
  assert.match(page, /result-multi-2128\.png/);
  assert.match(page, /result-multi-3128\.png/);
...
test("logo and viewport motion refinements remain stable", () => {
  ...
  assert.match(logo, /brand-logo-ai/);
  assert.match(logo, /brand-logo-a-dots/);
  assert.doesNotMatch(logo, /<img/);
  assert.match(css, /\.product-illustration[\s\S]*width: min\(900px, 100%\)/);
  assert.match(css, /\.hero-answer-option:hover/);
  assert.match(css, /@keyframes magnifier-drift/);
  assert.match(css, /@keyframes badge-float/);
  ...
  assert.doesNotMatch(css, /Iowan Old Style|Palatino Linotype|Book Antiqua|Georgia, serif/);
...
test("all required public assets exist", () => {
  ...
  assert.equal(
    existsSync(resolve(root, "public/zenaian-how-it-works-v2.png")),
    true,
  ...
  for (const name of [
    ...
    "result-multi-2128",
    "result-multi-3128",
    "result-inconclusive128",
    "result-error128",
  ]) {
    assert.equal(existsSync(resolve(root, `public/zenaian-icons/hero/${name}.png`)), true, `${name}.png missing`);
  }
});
