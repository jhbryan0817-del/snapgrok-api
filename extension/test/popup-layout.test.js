...
test("signed-out header centers the horizontal Zenaian product identity", () => {
  ...
  assert.match(popupCss, /\.app-header\.is-signed-out \{[\s\S]*?grid-template-columns: 34px max-content;[\s\S]*?column-gap: 10px;[\s\S]*?min-height: 70px;[\s\S]*?margin-bottom: 4px;[\s\S]*?padding: 6px 14px;[\s\S]*?justify-content: center;[\s\S]*?border-color: transparent;[\s\S]*?background: transparent;[\s\S]*?box-shadow: none;/);
