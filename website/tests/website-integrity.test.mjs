...
test("account readiness combines extension installation and server quota state", () => {
  ...
  assert.match(account, /account-page-heading[\s\S]*<h1 className="page-section-title">Account Settings<\/h1>[\s\S]*account-welcome account-status-only[\s\S]*<AccountReadiness \/>/);
