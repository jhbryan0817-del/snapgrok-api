...
const studyWorkflow = await readFile(
  new URL("../app/study-workflow-demo.tsx", import.meta.url),
  "utf8",
);
...
test("approved landing copy is present", () => {
  ...
  assert.match(studyWorkflow, /role="tablist"/);
  ...
  assert.match(studyWorkflow, /aria-label="Attach a file"/);
  assert.match(studyWorkflow, /aria-label="Send message"/);
  ...
  assert.match(page, /xAI, may retain API inputs and outputs for up to 30 days/);
