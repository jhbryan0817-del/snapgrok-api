...
test("redesigned landing page includes the requested product workflow", () => {
  ...
  assert.match(page, /Eliminate unnecessary steps - maximize your studying efficiency/);
  ...
  assert.match(page, /Powered by the optimal AI model - Grok 4\.5/);
...
test("hero shortcut, scan, and icon states share one cycle on desktop and reduced-motion devices", () => {
  ...
  assert.match(css, /hero-browser-demo \.demo-illustration-note[\s\S]*animation: none !important[\s\S]*opacity: \.96/);
