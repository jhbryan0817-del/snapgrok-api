...
async function fetchAnalysisWithAuthRetry({ serverUrl, authToken, requestBody, signal }) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    const message = response.status === 401
      ? "Your SneakSolve session is no longer active. Open the extension and sign in again."
  ...
  throw new Error("Your SneakSolve session is no longer active. Open the extension and sign in again.");
}
