...
export function createPostgresDeviceSessionStore({
  ...
  const database = pool || new Pool({
    ...
    application_name: "sneaksolve-extension-auth",
  });
  const ownsPool = !pool;
  ...
  return {
    ...
    async cleanupExpired(now) {
      const pairings = await database.query(
        `DELETE FROM extension_pairing_grants
         WHERE expires_at < $1 OR consumed_at < $1 - interval '1 hour'`,
        [now],
      );
      const sessions = await database.query(
        `DELETE FROM extension_device_sessions
         WHERE refresh_expires_at < $1 - interval '7 days'
            OR revoked_at < $1 - interval '7 days'`,
        [now],
      );
      return pairings.rowCount + sessions.rowCount;
    },
  };
}
