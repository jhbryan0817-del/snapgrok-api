(() => {
  ...
  function buildInstruction(userInstruction) {
    return [
      ...
      "The screenshot contains a multiple-choice problem. The problem may have one correct option or several correct options. Determine every defensible correct option from A, B, C, D, and E.",
      "If the screenshot is unreadable, incomplete, genuinely ambiguous, or does not support a defensible answer, mark it inconclusive.",
      'FINAL OUTPUT FORMAT: Return only one compact JSON object. For an answer, use {"status":"answered","answers":["A"]} or {"status":"answered","answers":["A","C"]}. For an inconclusive result, use {"status":"inconclusive","answers":[]}. Do not include explanations, markdown, code fences, or any other text.',
    ].filter(Boolean).join("\n\n");
  }
