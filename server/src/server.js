...
export function createZenaianServer({
  ...
  const cleanupTimer = setInterval(
    () => {
      ...
      void privacyService?.maintenance?.().catch((error) => {
...
function parseVersionedSecrets(value) {
  const entries = [];
  for (const item of parseCsv(value)) {
    ...
    if (!/^\d+$/.test(versionText) || !key) {
      throw new Error(
        "PRIVACY_ARCHIVE_PREVIOUS_HMAC_KEYS must use version:base64url entries.",
      );
    }
...
async function startServer() {
  ...
  server.listen(config.port, "0.0.0.0", () => {
    ...
    void server.privacyService?.maintenance?.().catch((error) => {
      logPrivacyMaintenanceError(error);
    });
  });
}
