...
test("the supplied Whop production catalog passes fail-closed configuration", () => {
  const config = createConfig({
    ...
    WHOP_PLUS_PLAN_ID: "plan_OFBOhk4mPJkao",
    WHOP_PLUS_PRODUCT_ID: "prod_KJzTb3dCClstC",
    WHOP_ULTRA_PLAN_ID: "plan_nqzuo5pPp6Y5w",
  ...
  assert.equal(config.whopPlusPlanId, "plan_OFBOhk4mPJkao");
  assert.equal(config.whopUltraPlanId, "plan_nqzuo5pPp6Y5w");
});
