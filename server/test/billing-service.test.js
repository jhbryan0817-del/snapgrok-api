...
function testConfig() {
  return {
    ...
    whopPlusPlanId: "plan_QzpD3pxTswPLX",
    whopPlusProductId: "prod_M3Wts8bsfX4mK",
    whopUltraPlanId: "plan_FZknYvJ1uz41F",
...
function fakeStore(overrides = {}) {
  return {
    ...
    async applyPaymentFailureWebhook() { return { duplicate: false, applied: true }; },
...
test("checkout pins the server-side Whop catalog and Clerk intent metadata", async () => {
  let providerInput;
  let intentInput;
  let marked;
  const service = createBillingService({
  ...
  assert.equal(intentInput.providerPlanId, "plan_FZknYvJ1uz41F");
  assert.equal(intentInput.productId, "prod_kIiWFLHOWhrWa");
  assert.equal(providerInput.planId, "plan_FZknYvJ1uz41F");
  assert.equal(providerInput.metadata.clerk_user_id, TESTER);
  assert.match(providerInput.metadata.checkout_intent_id, /^[0-9a-f-]{36}$/);
  assert.equal(providerInput.redirectUrl, "https://www.sneaksolve.com/account?billing=return");
  assert.equal(marked[0], providerInput.metadata.checkout_intent_id);
});
...
test("signed membership activation is normalized and bound to Clerk metadata", async () => {
  let applied;
  const service = createBillingService({
    config: testConfig(),
    store: fakeStore({
  ...
  assert.equal(applied.checkoutIntentId, "11111111-1111-4111-8111-111111111111");
...
test("payment.failed immediately removes paid access through the store", async () => {
  ...
  const service = createBillingService({
    config: testConfig(),
    store: fakeStore({
      async applyPaymentFailureWebhook(input) { failedInput = input; return { duplicate: false, applied: true }; },
    }),
    whopClient: fakeWhop(),
    now: () => NOW,
  });
  const result = await signedWebhook(service, "payment.failed", paymentResource("failed"));
  assert.equal(result.applied, true);
...
test("cancellation is server-side, at period end, and retains the expiry", async () => {
  ...
  const result = await service.cancelMembership({ userId: TESTER });
...
function paymentResource(status) {
  return {
