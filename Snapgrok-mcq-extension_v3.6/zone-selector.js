(() => {
  ...
  const LISTENER_KEY = "__snapgrokV36ZoneListenerInstalled";
  const CONTROLLER_KEY = "__snapgrokV36ZoneController";
  const DIALOG_ID = "__snapgrok-v36-selector-dialog";
  const SHADOW_HOST_ID = "__snapgrok-v36-selector-shadow-host";
  const BACKDROP_STYLE_ID = "__snapgrok-v36-selector-style";
  ...
  async function startSelector(operationId) {
    ...
    dialog.id = DIALOG_ID;
    dialog.setAttribute("aria-label", "SnapGrok screenshot area selector");
    ...
    crosshair.style.cssText = [
      ...
      "pointer-events: none !important",
      "z-index: 2 !important",
      "background: linear-gradient(rgba(255,255,255,0.96), rgba(255,255,255,0.96)) center / 22px 3px no-repeat, linear-gradient(rgba(255,255,255,0.96), rgba(255,255,255,0.96)) center / 3px 22px no-repeat, linear-gradient(rgba(48,48,48,0.98), rgba(48,48,48,0.98)) center / 20px 1px no-repeat, linear-gradient(rgba(48,48,48,0.98), rgba(48,48,48,0.98)) center / 1px 20px no-repeat !important",
  ...
  async function sendReliable(message) {
    ...
    console.error(
      `[SnapGrok V3.6] Selector message delivery failed: ${lastError?.message || "No acknowledgement"}`,
    );
    return false;
  }
