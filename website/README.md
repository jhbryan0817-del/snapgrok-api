# SneakSolve website v5.8.0

The SneakSolve marketing, authentication, pricing, and account website with
tester-only Whop sandbox billing controls and live account-readiness status.
...
- Checkout destinations must be exact HTTPS Whop URLs returned by the
  authenticated API. Cancellation is an authenticated server-side operation.
...
Use Node 22.13.1. See the root `DEPLOYMENT_GUIDE.md`; deploy API v5.5.0 before
website v5.8.0. Extension v5.2.2 remains unchanged.
