...
test("redesigned landing page includes the requested product workflow", () => {
  ...
  assert.match(page, /zenaian-how-it-works-compact\.png/);
...
test("logo and viewport motion refinements remain stable", () => {
  ...
  assert.match(logo, /<img src="\/zenaian-logo-full-v2\.png"/);
  ...
  assert.match(css, /\.hero-description[\s\S]*font-family: inherit/);
