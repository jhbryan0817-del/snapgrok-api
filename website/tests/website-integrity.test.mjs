...
test("redesigned landing page includes the requested product workflow", () => {
  ...
  assert.match(studyWorkflow, /useState<ManualTab>\("question"\)/);
  ...
  assert.match(page, /privacy-heading[\s\S]*<LockIcon \/>[\s\S]*Our #1 Priority: Privacy/);
  assert.match(page, /privacy-disposal-illustration/);
  assert.match(page, /Transient request data is released from active processing/);
  ...
  assert.match(page, /Our #1 Priority: Privacy/);
...
test("account readiness combines extension installation and server quota state", () => {
  ...
  assert.match(account, /localGreeting/);
  assert.match(account, /Good morning/);
  assert.match(account, /Good afternoon/);
  assert.match(account, /Good evening/);
  assert.doesNotMatch(account, /user\?\.firstName/);
  assert.match(account, /\{greeting\}\./);
  assert.doesNotMatch(account, /VIEW YOUR DETAILS/);
...
test("Use Cases and Careers pages are substantive and linked globally", () => {
  ...
  assert.match(careers, /global education SaaS company based in Seoul, Korea/);
  assert.match(careers, /OPEN POSITIONS/);
  for (const role of [
    "Product Designer",
  ...
  assert.match(careers, /Build the future of focused learning/);
...
test("privacy page covers the current product and production billing data flow", () => {
  ...
  assert.match(privacy, /retained for 30 days by default/);
  assert.match(privacy, /audit suspected abuse or[\s\S]*misuse/);
  assert.match(privacy, /https:\/\/docs\.x\.ai\/developers\/faq\/security/);
