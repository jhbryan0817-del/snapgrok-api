...
test("redesigned landing page includes the requested product workflow", () => {
  ...
  assert.match(page, /Explore Zenaian before you begin/);
  assert.doesNotMatch(page, /Receive your answers|Select or hover over an icon state|Comprehensive Customizability|Accelerate Test Preparation/);
...
test("privacy page covers the current product and production billing data flow", () => {
  ...
  assert.match(privacy, /qualified counsel/);
