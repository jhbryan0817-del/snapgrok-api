# SneakSolve v5.1.7 acceptance tests
...
28. Immediately capture a test question. Confirm the Render request reaches
    `/api/analyze` with a non-empty Bearer token and no longer returns
    `AUTH_REQUIRED`.
29. Confirm the native token metadata reports `azp` absent, then confirm the
    exact extension-origin request succeeds while the wrong-origin server test
    returns `AUTH_TOKEN_PARTY_INVALID`.
30. Confirm selected-area capture works on Google Search without a false
    active-page-layer error.
31. On `chrome://extensions`, confirm selected-area capture shows the controlled
    browser-owned-page error and full-screen capture remains available.
32. Confirm the Toolbar results preview is ordered: Single answer, Multiple
    answers, Processing, Inconclusive, Error.
33. Confirm the preview and actual toolbar use the same amber tile with a white
    question mark for inconclusive results and the same red tile with a white
    exclamation mark for errors.
34. Inspect an authenticated `/api/analyze` request and confirm it contains one
    UUID `operationId`, then confirm an xAI success consumes exactly one unit.
35. Force an xAI technical failure and confirm the reserved unit is released.
36. Use all five Free questions and confirm the sixth capture does not call xAI
    and the toolbar tooltip explains the daily limit and reset time.
37. Confirm multiple-answer results still display for six seconds, single
    answers still display for four seconds, and both capture modes and custom
    context behave exactly as v5.1.4.
38. Visit the signed-in production account page with v5.1.7 enabled and confirm
    the readiness light recognizes the extension. Disable the extension and
    refocus the account tab; confirm it changes to "Please download the
    extension." Confirm a non-allowlisted origin cannot obtain a ping response.
