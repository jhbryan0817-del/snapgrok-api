...
test("redesigned landing page includes the requested product workflow", () => {
  ...
  assert.match(page, /zenaian-how-it-works\.png/);
...
test("logo and viewport motion refinements remain stable", () => {
  ...
  assert.match(read("app/brand-logo.tsx"), /brand-name-ai/);
  assert.doesNotMatch(read("app/brand-logo.tsx"), /<img/);
...
test("authentication shell is intentionally simple", () => {
  ...
  assert.match(shell, /Welcome to <BrandName \/>\./);
...
test("all required public assets exist", () => {
  ...
  assert.equal(
    existsSync(resolve(root, "public/zenaian-how-it-works.png")),
    true,
