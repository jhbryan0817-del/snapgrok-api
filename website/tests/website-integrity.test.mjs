...
test("redesigned landing page includes the requested product workflow", () => {
  ...
  assert.match(answerToolbar, /answer-toolbar-default/);
  ...
  assert.match(answerToolbar, /activeId === null \? null : <img/);
  ...
  assert.match(page, /manual-tab-demo[\s\S]*Copy\. Switch\. Paste\. Return\./);
  assert.match(page, /zenaian-speed-demo[\s\S]*Capture\. Answer\. Keep moving\./);
...
test("answer-state hover boundaries remain stable and the study comparison has both motion demos", () => {
  ...
  assert.match(page, /className="study-motion-demo manual-tab-demo"/);
  assert.match(page, /className="study-motion-demo zenaian-speed-demo"/);
  assert.match(css, /grid-template-columns: minmax\(190px, 1fr\) minmax\(310px, 410px\) minmax\(190px, 1fr\)/);
  assert.match(css, /@keyframes manual-question-cycle/);
  assert.match(css, /@keyframes zenaian-mini-scan/);
  assert.match(css, /manual-screen-question[\s\S]*manual-question-cycle 6s ease-in-out infinite !important/);
  assert.match(css, /zenaian-motion-processing[\s\S]*zenaian-mini-processing 5\.4s linear infinite !important/);
});
...
test("account plan panel reads server-authoritative status and cancels through the API", () => {
  ...
  assert.match(panel, /billing-panel-heading[\s\S]*PLAN &amp; USAGE[\s\S]*onClick=\{\(\) => void refresh\(\)\}/);
  assert.match(panel, /className="billing-panel-actions"[\s\S]*href="\/pricing">View plans/);
  assert.doesNotMatch(panel, /AccountReadiness|billing-panel-footer-actions|billing-refresh-button/);
...
test("account readiness combines extension installation and server quota state", () => {
  ...
  assert.doesNotMatch(panel, /AccountReadiness/);
  ...
  assert.match(account, /account-header-status[\s\S]*<AccountReadiness \/>/);
  assert.match(account, /account-page-heading[\s\S]*<span className="section-kicker">VIEW YOUR DETAILS<\/span>[\s\S]*<h1 className="page-section-title">Account &amp; Settings<\/h1>/);
  assert.doesNotMatch(account, /account-welcome account-status-only/);
  assert.doesNotMatch(account, /Good morning|Good afternoon|Good evening|YOUR ZENAIAN ACCOUNT|Manage your profile/);
});
...
test("Use Cases and Careers pages are substantive and linked globally", () => {
  ...
  assert.match(useCases, /Complete Focus on the Questions/);
  assert.match(careers, /Build Zenaian Together/);
