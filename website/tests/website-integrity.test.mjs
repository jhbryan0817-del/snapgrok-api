...
test("logo and viewport motion refinements remain stable", () => {
  ...
  assert.match(logo, /<img src="\/zenaian-logo\.png"/);
...
test("the site uses the Zenaian icon as a favicon without changing the header logo", () => {
  const layout = read("app/layout.tsx");
  const headerLogo = read("app/brand-logo.tsx");
  assert.match(layout, /icons: \{ icon: "\/favicon\.png"/);
  assert.match(layout, /@fontsource\/eb-garamond\/latin-400\.css/);
  assert.equal(existsSync(resolve(root, "public/favicon.png")), true);
  assert.match(headerLogo, /src="\/zenaian-logo\.png"/);
});
