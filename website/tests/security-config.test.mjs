...
test("public pages use the Zenaian product name", async () => {
  for (const file of [
    "app/page.tsx",
...
test("production site, API, and Clerk origins are pinned", async () => {
  const provider = await fs.readFile("app/clerk-provider.tsx", "utf8");
  const config = await fs.readFile("next.config.ts", "utf8");
...
test("billing boundary is documented as server-authoritative", async () => {
  const security = await fs.readFile("SECURITY.md", "utf8");
  for (const expected of [
    /verified token/i,
    /must not trust/i,
    /allowlisted\s+Whop\s+plan and product IDs/i,
    /raw request bytes/i,
    /idempotently/i,
    /final authority/i,
  ]) {
