...
test("embedded Clerk profile stays open inside a fixed-height account section", () => {
  const account = read("app/account/page.tsx");
  const css = read("app/globals.css");
  assert.match(account, /<section[\s\S]*className="account-settings-section account-settings-static"[\s\S]*aria-labelledby="account-settings-title"/);
  assert.match(account, /<header className="account-settings-summary">/);
  assert.match(account, /className="account-settings-summary-title"[\s\S]*role="heading"[\s\S]*aria-level=\{2\}/);
  assert.doesNotMatch(account, /<details|<summary|account-settings-chevron/);
  ...
  assert.match(css, /height: min\(360px, calc\(100vh - 160px\)\) !important/);
  assert.doesNotMatch(css, /account-settings-disclosure|account-settings-chevron/);
});
