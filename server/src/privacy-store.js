      ...
      try {
        ...
        const recent = await database.query(
          `SELECT received_at
           ...
           LIMIT 1`,
          [userHmacCandidates(userId), new Date(at.getTime() - cooldownMs)],
        );
        if (recent.rows[0]) {
          const receivedAt = asDate(recent.rows[0].received_at) || at;
          const retryAfterSeconds = Math.max(
            1,
            Math.ceil((receivedAt.getTime() + cooldownMs - at.getTime()) / 1000),
          );
          const error = privacyStoreError(
            "A recent data export already exists. Please wait before requesting another.",
