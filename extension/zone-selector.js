(() => {
  ...
  const LISTENER_KEY = "__sneaksolveV512ZoneListenerInstalled";
  ...
  const START_MESSAGE = "SNAPGROK_START_ZONE_SELECTOR_V512";
  const STOP_MESSAGE = "SNAPGROK_STOP_ZONE_SELECTOR_V512";
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (
      ...
      message?.type === "SNAPGROK_STOP_ZONE_SELECTOR"
    ) {
    ...
    if (message?.type !== START_MESSAGE) return false;
  ...
  async function startSelector(operationId) {
    ...
    dialog.id = DIALOG_ID;
    dialog.setAttribute("aria-label", "Zenaian screenshot area selector");
    ...
    if (!dialog.open) {
      cleanup(false);
      throw new Error("Chrome closed the screenshot selector before it became active.");
    }

    let selectorMatchesModalState = false;
    try {
      selectorMatchesModalState = dialog.matches(":modal");
    } catch {}

    const selectorFillsViewport =
      rect.width >= Math.max(10, window.innerWidth - 2) &&
      rect.height >= Math.max(10, window.innerHeight - 2);

    // showModal() completed and dialog.open is true, which is the authoritative
    // activation result. Chrome can report :modal, exact geometry, or
    // elementFromPoint() inconsistently from an extension's isolated world on
    // complex pages such as Google Search. These probes are useful diagnostics,
    // but rejecting the selector here destroys an overlay Chrome already opened.
    if (
      !selectorMatchesModalState ||
      !selectorFillsViewport ||
      !selectorOwnsHitPoint
    ) {
      console.debug("[Zenaian V5.3] Selector probe differed; continuing.", {
        open: dialog.open,
        modal: selectorMatchesModalState,
        fillsViewport: selectorFillsViewport,
        ownsHitPoint: selectorOwnsHitPoint,
  ...
  async function sendReliable(message) {
    ...
    console.error(
      `[Zenaian V5] Selector message delivery failed: ${lastError?.message || "No acknowledgement"}`,
    );
    return false;
  }
