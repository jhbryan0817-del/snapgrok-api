    ...
    async reconcile() {
      ...
      try {
        ...
        const payments = await whopClient.listPaymentsSince(updatedAfter, {
          planIds: [config.whopPlusPlanId, config.whopUltraPlanId],
        });
        ...
        for (const resource of payments) {
          if (!isConfiguredCatalogPayment(config, resource)) continue;
          try {
...
function normalizePayment(config, resource, eventName) {
  ...
  return {
    ...
    substatus,
    displayStatus: paymentDisplayStatus(substatus),
...
function isConfiguredCatalogPayment(config, resource) {
  return Boolean(
    resource &&
