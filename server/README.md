...
| `GET/DELETE /api/extension/session` | Extension device session | Check or revoke the current device session |
| `POST /api/analyze-jobs` | Extension device session | Reserve quota and start transient analysis |
| `GET/DELETE /api/analyze-jobs/:id` | Same extension device session | Poll or cancel transient analysis |
