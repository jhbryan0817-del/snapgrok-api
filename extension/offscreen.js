...
async function fetchAnalysisWithAuthRetry({ serverUrl, authToken, requestBody, signal }) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    const message = response.status === 401
      ? authenticationFailureMessage(code, payload.requestId)
      : backendFailureMessage(code, payload, response.status);
  ...
  throw new Error(
    "[AUTH_REQUIRED] SneakSolve could not verify your sign-in. Open the extension and sign in again.",
  );
}
...
function authenticationFailureMessage(code, requestId) {
  const messages = {
    AUTH_SESSION_INACTIVE:
      "Your SneakSolve session was signed out or revoked. Open the extension and sign in again.",
    AUTH_TOKEN_EXPIRED:
      "Your SneakSolve session token expired and could not be refreshed. Open the extension and sign in again.",
    AUTH_TOKEN_PARTY_INVALID:
      "SneakSolve authentication is not configured for this extension build.",
    AUTH_TOKEN_INVALID:
      "SneakSolve could not verify this production Clerk session.",
    AUTH_REQUIRED:
      "SneakSolve could not verify your sign-in. Open the extension and sign in again.",
  };
  const message = messages[code] || messages.AUTH_REQUIRED;
  const safeRequestId = /^[a-f0-9-]{20,50}$/i.test(String(requestId || ""))
    ? `; request ${requestId}`
    : "";
  return `[${code}${safeRequestId}] ${message}`;
}

function backendFailureMessage(code, payload, status) {
  if (code === "QUOTA_EXHAUSTED") {
    const reset = formatQuotaReset(payload?.quota?.resetsAt);
    const limit = payload?.quota?.plan === "free"
      ? "Your five free questions for today have been used."
      : "Your monthly SneakSolve question allowance has been used.";
    return `[${code}] ${limit}${reset} Open your SneakSolve account to review plan usage.`;
  }
  if (
    code === "BILLING_DATABASE_UNAVAILABLE" ||
    code === "BILLING_PROVIDER_UNAVAILABLE"
  ) {
    return `[${code}] SneakSolve plan verification is temporarily unavailable. Please try again shortly.`;
  }
  if (
    code === "ANALYSIS_ALREADY_CONSUMED" ||
    code === "ANALYSIS_OPERATION_RELEASED"
  ) {
    return `[${code}] This capture request has already been handled. Capture the question again.`;
  }
  const message =
...
function formatQuotaReset(value) {
  ...
  return ` It resets ${date.toLocaleString()}.`;
}
