...
// Reconciliation must keep recognizing retired plans until every membership
// and payment created under them has reached a terminal state. Checkout never
// calls this helper; whopCatalogForPlan intentionally returns current IDs only.
export function acceptedWhopPlanIds(config) {
  return [...new Set([
    config.whopPlusPlanId,
    ...(config.whopPlusLegacyPlanIds || []),
    config.whopUltraPlanId,
...
function isAcceptedPlanId(planId, currentPlanId, legacyPlanIds) {
  return (
    planId === currentPlanId ||
    Boolean(legacyPlanIds?.has?.(planId))
  );
}
