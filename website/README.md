...
The hero uses one ten-second demonstration timeline. Its bare-text illustration
notice appears only after the shortcut preview leaves, remains through
Processing and answer B, and disappears when the default Z returns. The
answer-state explorer uses fixed geometry and pre-rendered icon layers. The
study workflow combines an interactive manual tab preview, a narrow center
flowchart, and a shortcut-to-answer Zenaian sequence. The account's Clerk
profile controls remain available through an initially collapsed disclosure.
