...
export function AccountReadiness() {
  ...
  return (
    <div
      className={`account-status-card account-status-${readiness.tone} account-status-kind-${readiness.kind}`}
      aria-live="polite"
    >
...
function resolveReadiness({
  ...
  if (extensionState === "missing") {
    return {
      ...
      detail: "Install the extension to continue.",
    } as const;
  }
  ...
  if (extensionState === "installed") {
    return {
      ...
      detail: connectionError || "Open the extension and try again.",
    } as const;
  }
  ...
  return {
    ...
    detail: error || "Refresh the plan details to try again.",
  } as const;
}
