...
chrome.runtime.onMessageExternal.addListener((message, sender, sendResponse) => {
  if (message?.type !== "SNEAKSOLVE_EXTENSION_PING") return false;
  ...
  if (senderOrigin !== WEBSITE_ORIGIN) return false;
    ...
    SnapGrokAuth.getAuthSnapshot()
      .then((snapshot) => {
        sendResponse({
          ...
          snapshot: {
            ...
            syncCookiePresent: snapshot.syncCookiePresent,
      ...
      .catch((error) => {
        trace("AUTH_SNAPSHOT_FAILED", {
          message: String(error?.message || "Unknown Clerk error").slice(0, 160),
        });
...
function trace(eventName, details = {}) {
  ...
  // output, and answers.
  console.debug(`[SneakSolve V5] ${eventName}`, details);
}
...
async function stopActiveOperationForSignedOutSession() {
  ...
  await chrome.runtime.sendMessage({
    ...
    operationId: operation.id,
    reason: "Your SneakSolve session ended. Sign in again before capturing another question.",
  }).catch(() => undefined);
  ...
  await displayOutcome(
    operation.id,
    errorOutcome(),
    new Error("Your SneakSolve session ended. Sign in again before capturing another question."),
  );
}
...
async function handleCommand(command) {
  ...
  if (!authToken) {
    throw new Error("Sign in to SneakSolve from the extension popup before capturing a question.");
  }
  ...
  try {
    ...
    if (command === COMMAND_FULL) {
      ...
      await setProcessingIndicator(
        operation.id,
        "SneakSolve captured the visible tab and is analyzing it…",
      );
    ...
    await chrome.action.setTitle({ title: "SneakSolve: drag to select a screenshot area" });
...
async function handleZoneSelected(message, sender) {
  ...
  try {
    ...
    await setProcessingIndicator(
      operation.id,
      "SneakSolve captured the selected area and is analyzing it…",
    );
...
async function analyzeAndDisplay(operationId, imageDataUrl, userInstruction, existingAuthToken = "") {
  ...
  try {
    ...
    if (!authToken) {
      throw new Error("Your SneakSolve session expired. Open the extension and sign in again.");
    }
...
async function closeOffscreenDocumentQuietly() {
  try {
    ...
    console.debug(`[SneakSolve V5] Offscreen document cleanup skipped: ${error?.message || "Unknown error"}`);
  }
}
...
async function displayOutcome(operationId, rawOutcome, error = null) {
  ...
  if (error) {
    console.error(
      `[SneakSolve V5] ${error?.name || "Error"}: ${error?.message || "Unknown error"}`,
    );
  }
...
function titleForOutcome(outcome) {
  const displaySeconds = resultDisplayDuration(outcome) / 1000;
  const resetText = `resets in ${displaySeconds} seconds`;

  if (outcome.status === "inconclusive") {
    return `SneakSolve: inconclusive · ${resetText}`;
  }

  if (outcome.status === "error") {
    return `SneakSolve system error · ${resetText}`;
  }
...
function titleForSystemError(message, displayDurationMs) {
  const safeMessage = String(message || "The request could not be completed.")
    ...
    .trim()
    .slice(0, 180);
  return `SneakSolve: ${safeMessage} · resets in ${displayDurationMs / 1000} seconds`;
}
...
async function recoverAndCheckOccupied() {
  ...
  if (operation.phase === "processing" || operation.phase === "capturing") {
    await chrome.action.setIcon({ path: ICONS.processing });
    await chrome.action.setTitle({ title: "SneakSolve is processing the captured screenshot…" });
    return true;
  }
...
async function setDefaultIcon() {
  ...
  await chrome.action.setTitle({ title: "Open SneakSolve settings" });
}
