...
test("account privacy controls use authenticated server routes", () => {
  const account = read("app/account/page.tsx");
  const panel = read("app/account/privacy-panel.tsx");
  const api = read("app/privacy-api.ts");

  assert.match(account, /<PrivacyPanel \/>/);
  for (const label of ["View my data", "Download JSON", "Delete account"]) {
  ...
  assert.match(panel, /sign out and sign in again/);
  assert.doesNotMatch(panel, /reason|Why are you deleting/i);
});
