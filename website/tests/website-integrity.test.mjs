...
test("redesigned landing page includes the requested product workflow", () => {
  ...
  assert.match(page, /Stay in your study flow while Zenaian removes/);
...
test("pricing retains the approved plans and uses the secure checkout client", () => {
  ...
  const css = read("app/globals.css");
  assert.match(pricing, /<span className="section-kicker">PLAN UPGRADES<\/span>[\s\S]*<h1 className="page-section-title">Pick What Suits You<\/h1>/);
  assert.doesNotMatch(pricing, /SIMPLE PRICING|Start with five questions a day/);
...
test("account plan panel reads server-authoritative status and cancels through the API", () => {
  ...
  assert.match(panel, /billing-panel-heading[\s\S]*PLAN &amp; USAGE[\s\S]*<AccountReadiness \/>/);
  assert.match(panel, /billing-panel-footer-actions[\s\S]*href="\/pricing">View plans[\s\S]*className="billing-refresh-button"/);
...
test("account readiness combines extension installation and server quota state", () => {
  ...
  assert.match(panel, /<AccountReadiness \/>/);
  ...
  assert.match(account, /account-page-heading[\s\S]*<span className="section-kicker">SETTINGS<\/span>[\s\S]*<h1 className="page-section-title">Manage Your Account<\/h1>/);
  assert.doesNotMatch(account, /account-welcome account-status-only|<AccountReadiness \/>/);
