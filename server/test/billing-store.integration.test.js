...
async function verifyCheckoutAndMembershipMapping(store) {
  ...
  const ultraIntentId = randomUUID();
  await store.createCheckoutIntent({
    id: ultraIntentId,
    userId,
    planCode: "ultra",
  ...
  await store.markCheckoutCreated(
    ultraIntentId,
    "ch_ultracheckout123456",
    "https://sandbox.whop.com/checkout/ultra?session=ch_ultracheckout123456",
  );
  const ultraMembership = normalizedMembership({
    id: "mem_ultramembership123456",
    productId: "prod_kIiWFLHOWhrWa",
    planId: "plan_FZknYvJ1uz41F",
    planCode: "ultra",
    checkoutConfigurationId: "ch_ultracheckout123456",
  });
  assert.equal((await store.applyMembershipWebhook({
    ...input,
    deliveryId: "msg_ultra123456",
    payloadDigest: "d".repeat(64),
    membership: ultraMembership,
    checkoutIntentId: ultraIntentId,
  })).applied, true);
  ...
  await assert.rejects(
    store.createCheckoutIntent({
      ...
      providerPlanId: membership.planId,
      expiresAt,
    }),
