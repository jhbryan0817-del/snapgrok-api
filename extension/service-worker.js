  ...
  try {
    ...
    if (command === COMMAND_FULL) {
      ...
      await setProcessingIndicator(
        operation.id,
        "Zenaian captured the visible tab and is analyzing it…",
      );
...
async function handleZoneSelected(message, sender) {
  ...
  try {
    ...
    await setProcessingIndicator(
      operation.id,
      "Zenaian captured the selected area and is analyzing it…",
    );
...
function titleForOutcome(outcome) {
  ...
  if (outcome.status === "inconclusive") {
    return `Zenaian: inconclusive · ${resetText}`;
  }

  if (outcome.status === "error") {
    return `Zenaian system error · ${resetText}`;
  }
...
async function recoverAndCheckOccupied() {
  ...
  if (operation.phase === "processing" || operation.phase === "capturing") {
    ...
    await chrome.action.setTitle({ title: "Zenaian is processing the captured screenshot…" });
    return true;
  }
...
async function setDefaultIcon() {
  ...
  await chrome.action.setTitle({ title: "Open Zenaian settings" });
}
