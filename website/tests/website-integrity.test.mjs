...
test("redesigned landing page includes the requested product workflow", () => {
  ...
  assert.match(answerToolbar, /useState[\s\S]*\("processing"\)/);
...
test("hero icon keeps cycling for reduced-motion visitors and uses the compact frame", () => {
  const css = read("app/globals.css");
  assert.match(css, /--icon-duration: 12s/);
  assert.match(css, /aspect-ratio: 16 \/ 7\.25/);
  assert.match(css, /demo-icon-idle-swap var\(--icon-duration\) linear infinite !important/);
  assert.match(css, /demo-icon-processing-swap var\(--icon-duration\) linear infinite !important/);
  assert.match(css, /demo-icon-answer-swap var\(--icon-duration\) linear infinite !important/);
  assert.match(css, /@keyframes demo-icon-answer-swap/);
});
