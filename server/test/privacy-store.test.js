...
test("retention purge locks every candidate and rechecks mutable eligibility", async () => {
  ...
  assert.match(
    analysisUsage,
    /target\.settled_at < \$1::timestamptz - interval '30 days'/,
  );
  ...
  assert.match(
    providerEvents,
    /event\.received_at < \$1::timestamptz - interval '30 days'/,
  );
  ...
  assert.match(
    payments,
    /payment\.provider_updated_at <\s+\$1::timestamptz - interval '12 months'/,
  );
...
test("privacy exports remain immediately retryable without storing a payload", async () => {
  const queries = [];
  const pool = scriptedPool((sql) => {
    queries.push(sql);
    return {
      rows: [],
      rowCount: sql.startsWith("INSERT INTO privacy_request_audit") ? 1 : 0,
    };
  });
  const store = privacyStore(pool);

  const firstRequestId = await store.beginExport(USER_ID, NOW);
  const secondRequestId = await store.beginExport(
    USER_ID,
    new Date(NOW.getTime() + 1_000),
  );
  assert.match(firstRequestId, /^[0-9a-f-]{36}$/);
  assert.match(secondRequestId, /^[0-9a-f-]{36}$/);
  assert.notEqual(firstRequestId, secondRequestId);
  assert.equal(
    queries.filter((sql) =>
      sql.startsWith("INSERT INTO privacy_request_audit")).length,
    2,
  );
  assert.equal(
    queries.some((sql) =>
      sql.startsWith("SELECT received_at FROM privacy_request_audit")),
    false,
  );
});

test("privacy audit completion casts timestamps before interval arithmetic", async () => {
  const queries = [];
  const pool = scriptedPool((sql) => {
    queries.push(sql);
    return { rows: [], rowCount: 1 };
  ...
  const completion = queries.find((sql) =>
    sql.startsWith("UPDATE privacy_request_audit SET state = $2"));
  assert.ok(completion);
  assert.match(
    completion,
    /THEN \$4::timestamptz \+ interval '1 year'/,
  );
  assert.doesNotMatch(completion, /THEN \$4 \+ interval/);
});

test("ZDR safety failures atomically disable the persisted latch", async () => {
  const queries = [];
  const pool = scriptedPool((sql, parameters) => {
  ...
  const state = await store.recordZdrFailure(3, NOW);
