...
The hero uses one ten-second demonstration timeline. Its illustration notice
appears only after the shortcut preview leaves, remains through Processing and
answer B, and disappears when the default Z returns. The answer-state explorer
uses a stable shared hover boundary, and the study workflow combines a compact
center flowchart with separate manual tab-switching and Zenaian capture demos.
The account's Clerk profile controls remain available through an initially
