...
export function PrivacyPanel() {
  ...
  async function authenticatedToken(): Promise<string> {
    ...
    if (!token) {
      throw new PrivacyApiError(
        401,
        "AUTH_REQUIRED",
        "Please sign out and sign in again before using privacy controls.",
      );
    }
    return token;
  }
  ...
  async function viewData() {
    ...
    try {
      ...
      setSummary(await getPrivacySummary(token));
      setSummaryOpen(true);
    } catch (nextError) {
  ...
  async function downloadData() {
    ...
    try {
      const token = await authenticatedToken();
      const exportData = await getPrivacyExport(token);
...
function DeleteAccountModal({
  ...
  async function submitDeletion() {
    ...
    try {
      ...
      const nextReceipt = await deletePrivacyAccount(token, {
...
function privacyErrorMessage(error: unknown): string {
  if (
    ...
    return "For your security, sign out and sign in again, then retry this privacy request.";
  }
