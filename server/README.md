# SneakSolve API v5.5.0

Render-ready API with production Clerk authentication, exact-origin CORS,
PostgreSQL quotas, server-created Whop sandbox checkout, signed webhook
...
| `POST /api/billing/webhook` | Standard Webhooks signature | Apply Whop membership/payment state |
...
- `payment.failed` immediately falls back to Free. End-of-period cancellation
  keeps access through the current `renewal_period_end`.
